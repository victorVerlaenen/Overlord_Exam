...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float3 halfAngle = normalize(gLightDirection + viewDirection);
	float blinnTerm = dot(normal, halfAngle);
	blinnTerm = saturate(blinnTerm);
	blinnTerm = pow(blinnTerm, gShininess);
	
	float output = gColorSpecular * specIntensity * blinnTerm;
	return output;
}
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float3 reflectedDirection = reflect(-gLightDirection, normal);
	float phongTerm = dot(reflectedDirection, viewDirection);
	phongTerm = saturate(phongTerm);
	phongTerm = pow(phongTerm, gShininess);
	
	float output = gColorSpecular * specIntensity * phongTerm;
	return output;
}
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	sampledNormal = (sampledNormal * 2.0f) - float3(1.0f, 1.0f, 1.0f);
...
float3 CalculateDiffuse(float3 normal, float2 texCoord)
{
	...
	float3 diffColor = gColorDiffuse.rgb * diffuseStrength;
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	...
	return (environmentColor * (fresnel * fresnelMask));
}
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	float3 reflectedColor = gCubeEnvironment.Sample(gTextureSampler, reflectedVector) * gReflectionStrength;
	
	float3 refractedVector = refract(viewDirection, normal, gRefractionIndex) ;
	float3 refractedColor = gCubeEnvironment.Sample(gTextureSampler, refractedVector) * gRefractionStrength;
