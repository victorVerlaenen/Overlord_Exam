...
void SoftwareSkinningScene_3::Update()
{
	...
	{
		const XMVECTOR transformedVertexBone0{ XMVector3TransformCoord(XMLoadFloat3(&m_SkinnedVertices[i].originalVertex.Position), bone0transform) };
		const XMVECTOR transformedVertexBone1{ XMVector3TransformCoord(XMLoadFloat3(&m_SkinnedVertices[i].originalVertex.Position), bone1transform) };
		XMStoreFloat3(&m_SkinnedVertices[i].transformedVertex.Position, transformedVertexBone0 * m_SkinnedVertices[i].blendWeight0 + transformedVertexBone1 * m_SkinnedVertices[i].blendWeight1);
	}
