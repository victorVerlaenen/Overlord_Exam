...
float4 ShadowMapVS(float3 position:POSITION):SV_POSITION
{
	...
	float4 transformedPosition = mul(float4(position, 1.0f), mul(gWorld, gLightViewProj));
...
float4 ShadowMapVS_Skinned(float3 position:POSITION, float4 BoneIndices : BLENDINDICES, float4 BoneWeights : BLENDWEIGHTS) : SV_POSITION
{
	...
	float4 transformedPosition = float4(position, 1.0f);
	
	for(int i = 0; i < 4; ++i)
	{
		transformedPosition += mul(float4(position, 1.0f), gBones[BoneIndices[i]] * BoneWeights[i]);
	}
