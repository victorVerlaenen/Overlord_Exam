...
void SpriteComponent::Draw(const SceneContext& /*sceneContext*/)
{
	...
	auto pTranformComponent = this->GetGameObject()->GetTransform();
	...
	SpriteRenderer::Get()->AppendSprite(m_pTexture,
		DirectX::XMFLOAT2{ pTranformComponent->GetWorldPosition().x, pTranformComponent->GetWorldPosition().y },
		m_Color,
		m_Pivot,
		DirectX::XMFLOAT2{ pTranformComponent->GetScale().x, pTranformComponent->GetScale().y },
		MathHelper::QuaternionToEuler(pTranformComponent->GetWorldRotation()).z,
		pTranformComponent->GetPosition().z);
