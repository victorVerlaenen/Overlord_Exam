...
void Character::Update(const SceneContext& sceneContext)
{
	...
	if (true)
	{
		...
		if (sceneContext.pInput->IsMouseButton(InputState::down, VK_LBUTTON))
		{
			...
			look = XMFLOAT2(static_cast<float>(sceneContext.pInput->GetMouseMovement().x), static_cast<float>(sceneContext.pInput->GetMouseMovement().y));
		}
		...
		transformComponent->Rotate(m_TotalPitch, m_TotalYaw, 0.0f);
