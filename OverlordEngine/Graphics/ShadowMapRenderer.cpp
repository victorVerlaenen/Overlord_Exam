...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	//		*nearZ>0.1f
	//		*farZ>500.f
	const auto projectionMatrix = XMMatrixOrthographicLH(100.f * sceneContext.aspectRatio, 100.f, 0.1f, 1000.f);
	...
	const auto directionalLight = sceneContext.pLights->GetDirectionalLight();
	const auto lightPosition = directionalLight.position;
	const auto lightDirection = directionalLight.direction;
	//		*focusPosition: Calculate using the Direction Light position and direction
	const auto focusPosition = XMVectorAdd(XMLoadFloat4(&lightPosition), XMLoadFloat4(&lightDirection));
	constexpr auto up = XMFLOAT3{ 0,1,0 };
	const auto viewMatrix = XMMatrixLookAtLH(XMLoadFloat4(&directionalLight.position), focusPosition, XMLoadFloat3(&up));
...
void ShadowMapRenderer::DrawMesh(const SceneContext& sceneContext, MeshFilter* pMeshFilter, const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& meshBones)
{
	...
	const MaterialTechniqueContext techniqueContext = m_GeneratorTechniqueContexts[index];
	...
	{
		m_pShadowMapGenerator->SetVariable_MatrixArray(L"gBones", &meshBones[0].m[0][0], static_cast<UINT>(meshBones.size()));
	}
	...
	for (const auto& subMesh : pMeshFilter->GetMeshes())
	{
		...
		const int bufferId = pMeshFilter->GetVertexBufferId(techniqueContext.inputLayoutID, subMesh.id);
		const VertexBufferData& vertexBufferData = subMesh.buffers.vertexbuffers[bufferId];
		...
		const auto tech = techniqueContext.pTechnique;
